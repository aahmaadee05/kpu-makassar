        ...
        */
        public function login($username, $password) {  
                ...
                if($query->num_rows() == 1) {  
                        ...
                        $row   = $this->CI->db->query('SELECT id,nama,level,id_dapil,id_kecamatan,id_kelurahan,id_tps FROM users where username = "'.$username.'"');  
                        $admin         = $row->row();  
                        $id     = $admin->id; 
                        $nama   =$admin->nama; 
                        $level   =$admin->level;
                        $id_dapil   =$admin->id_dapil;
                        $id_kecamatan   =$admin->id_kecamatan;
                        $id_kelurahan   =$admin->id_kelurahan;
                        $id_tps   =$admin->id_tps;
                        ...
                        $this->CI->session->set_userdata('id_dapil', $id_dapil);
                        $this->CI->session->set_userdata('id_kecamatan', $id_kecamatan);
                        $this->CI->session->set_userdata('id_kelurahan', $id_kelurahan);
                        $this->CI->session->set_userdata('id_tps', $id_tps);
                        $this->CI->session->set_userdata('id', $id);

                        if ($this->CI->session->userdata('level')=='Admin') {
                                redirect(site_url('backend'));
                        }
                        elseif ($this->CI->session->userdata('level')=='User') {
